...
// Converte URL para usar proxy
function toProxyUrl(url: string): string {
  // Se é uma URL do GitHub, usa o proxy
  if (url.includes('formigteen.github.io') || url.includes('FormigTeen.github.io')) {
    return `${PROXY_URL}?path=${encodeURIComponent(url)}`;
  }
  // Se é um path relativo, constrói a URL completa
  if (!url.startsWith('http')) {
    const fullUrl = `${API_BASE_URL}/${url.replace(/^\//, '')}`;
    return `${PROXY_URL}?path=${encodeURIComponent(fullUrl)}`;
  }
  return `${PROXY_URL}?path=${encodeURIComponent(url)}`;
}
